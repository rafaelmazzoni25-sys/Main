...
float TerrainGrassWind1[TERRAIN_SIZE * TERRAIN_SIZE];
...
float WaterMove;
int CurrentLayer;
float g_fSpecialHeight = 1200.0f;

#ifdef DYNAMIC_FRUSTRUM
FrustrumMap_t	g_FrustrumMap;
#endif //DYNAMIC_FRUSTRUM

const float g_fMinHeight = -500.0f;
const float g_fMaxHeight = 1000.0f;
...
void ExitProgram()
{
	MessageBox(g_hWnd, GlobalText[11], nullptr, MB_OK);
...
bool SaveTerrainAttribute(char* FileName, int iMap)
{
	...
	fwrite(&iMap, sizeof(int), 1, fp);
	fwrite(&Width, sizeof(BYTE), 1, fp);
	fwrite(&Height, sizeof(BYTE), 1, fp);
	fwrite(TerrainWall, sizeof(WORD), TERRAIN_SIZE * TERRAIN_SIZE, fp);
...
void AddTerrainAttributeRange(int x, int y, int dx, int dy, BYTE att, BYTE Add)
{
	for (int j = 0; j < dy; ++j)
	{
		for (int i = 0; i < dx; ++i)
		{
			...
			if (Add)
			{
				AddTerrainAttribute(posX, posY, att);
			}
			else
			{
				SubTerrainAttribute(posX, posY, att);
			}
		}
	}
}
...
int OpenTerrainMapping(char* FileName)
{
	...
	int DataPtr = 0;
	DataPtr += 1;
	int iMapNumber = static_cast<int>(Data[DataPtr]); DataPtr += 1;
	...
	for (int i = 0; i < TERRAIN_SIZE * TERRAIN_SIZE; i++)
	{
		BYTE Alpha = Data[DataPtr]; DataPtr += 1;
...
bool SaveTerrainMapping(char* FileName, int iMapNumber)
{
	...
	for (int i = 0; i < TERRAIN_SIZE * TERRAIN_SIZE; i++)
	{
		unsigned char Alpha = static_cast<unsigned char>(TerrainMappingAlpha[i] * 255.f);
		fwrite(&Alpha, 1, 1, fp);
	}
	fclose(fp);
	{
	...
	else
	{
		...
		else
		{
			...
			case 0:
				...
				{
					Width = (float)GetScreenWidth() / 540.f * 9.1f * 0.404998f;
				}
				else if (g_Direction.m_CKanturu.IsMayaScene())
				{
					Width = (float)GetScreenWidth() / 540.f * 10.0f * 0.115f;
				}
				else
				{
					Width = (float)GetScreenWidth() / 540.f;
				}
