...
BASE_TILE_FILES: List[Tuple[str, ...]] = [
    ("TileGrass01.jpg", "TileGrass01.tga"),
    ("TileGrass02.jpg", "TileGrass02.tga"),
    ("AlphaTileGround01.Tga", "TileGround01.jpg"),
    ("AlphaTileGround02.Tga", "TileGround02.jpg"),
    ("AlphaTileGround03.Tga", "TileGround03.jpg"),
    ("TileWater01.jpg",),
    ("TileWood01.jpg",),
    ("TileRock01.jpg",),
    ("TileRock02.jpg",),
    ("TileRock03.jpg",),
    ("AlphaTile01.Tga", "TileRock04.jpg"),
    ("Object64/song_lava1.jpg", "TileRock05.jpg"),
    ("AlphaTile01.Tga", "TileRock06.jpg"),
    ("TileRock07.jpg",),
]

EXT_TILE_PATTERNS = (
    "ExtTile{index:02d}.jpg",
    "ExtTile{index}.jpg",
    "ExtTile{index:02d}.tga",
)
...
class TerrainData:
    def __init__(
        ...
        tile_layer1: Optional[np.ndarray] = None,
        tile_layer2: Optional[np.ndarray] = None,
        tile_alpha: Optional[np.ndarray] = None,
        tile_images: Optional[Sequence[QtGui.QImage]] = None,
    ):
        ...
        self.tile_layer1 = tile_layer1.astype(np.uint8) if tile_layer1 is not None else None
        self.tile_layer2 = tile_layer2.astype(np.uint8) if tile_layer2 is not None else None
        self.tile_alpha = tile_alpha.astype(np.float32) if tile_alpha is not None else None
        self.tile_images = list(tile_images) if tile_images else []
    ...
    def _build_positions(self) -> np.ndarray:
        ...
        texcoords = np.zeros((size * size, 2), dtype=np.float32)
        tilecoords = np.zeros_like(texcoords)
        idx = 0
        for y in range(size):
            for x in range(size):
                positions[idx] = (x * self.scale, self.heights[y, x], y * self.scale)
                normals[idx] = self._normals[y, x]
                texcoords[idx] = (x * 0.25, y * 0.25)
                tilecoords[idx] = (float(x), float(y))
                idx += 1
        self._flat_normals = normals
        self._texcoords = texcoords
        self._tilecoords = tilecoords
        return positions
    ...
    @property
    def tilecoords(self) -> np.ndarray:
    ...
    def has_tile_mapping(self) -> bool:
        return (
            self.tile_layer1 is not None
            and self.tile_layer2 is not None
            and self.tile_alpha is not None
            and len(self.tile_images) > 0
        )

    def tile_indices_at(self, x: float, z: float) -> Optional[Tuple[int, int, float]]:
        if not self.has_tile_mapping():
            return None
        xi = int(x / self.scale)
        zi = int(z / self.scale)
        if xi < 0 or zi < 0 or xi >= self.size or zi >= self.size:
            return None
        layer1 = int(self.tile_layer1[zi, xi])
        layer2 = int(self.tile_layer2[zi, xi])
        alpha = float(self.tile_alpha[zi, xi])
        return layer1, layer2, alpha

    def tile_texture_count(self) -> int:
...
class TerrainRepository:
    ...
    def load_map(self, descriptor: MapDescriptor, variant: str) -> TerrainData:
        ...
        layer1, layer2, alpha = self._load_tile_mapping(descriptor)
        tile_images = self._load_tile_textures(descriptor)
        return TerrainData(
            descriptor,
            heights,
            attributes,
    ...
    def _load_tile_mapping(
        self, descriptor: MapDescriptor
    ) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]:
        world_path = descriptor.world_path
        candidates = [
            world_path / f"EncTerrain{descriptor.world_index}.map",
            world_path / f"EncTerraintest{descriptor.world_index}.map",
        ]
        for path in candidates:
            if path.exists():
                return self._read_tile_mapping(path)
        return None, None, None

    def _read_tile_mapping(
        self, path: Path
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        raw = path.read_bytes()
        data = map_file_decrypt(raw)
        expected = 2 + TERRAIN_SIZE * TERRAIN_SIZE * 3
        if len(data) < expected:
            raise ValueError(f"Arquivo de mapeamento muito pequeno: {path}")
        offset = 2
        size = TERRAIN_SIZE * TERRAIN_SIZE
        layer1 = np.frombuffer(data, dtype=np.uint8, count=size, offset=offset)
        offset += size
        layer2 = np.frombuffer(data, dtype=np.uint8, count=size, offset=offset)
        offset += size
        alpha = np.frombuffer(data, dtype=np.uint8, count=size, offset=offset)
        layer1 = layer1.reshape((TERRAIN_SIZE, TERRAIN_SIZE)).copy()
        layer2 = layer2.reshape((TERRAIN_SIZE, TERRAIN_SIZE)).copy()
        alpha = alpha.reshape((TERRAIN_SIZE, TERRAIN_SIZE)).astype(np.float32) / 255.0
        return layer1, layer2, alpha

    def _load_tile_textures(self, descriptor: MapDescriptor) -> List[QtGui.QImage]:
        if self._data_root is None:
            return []
        images: List[QtGui.QImage] = []
        for names in BASE_TILE_FILES:
            image = self._load_first_tile_image(descriptor, names)
            if image is None:
                image = self._placeholder_tile()
            images.append(image)
        for i in range(1, 17):
            patterns = [pattern.format(index=i) for pattern in EXT_TILE_PATTERNS]
            image = self._load_first_tile_image(descriptor, patterns)
            if image is not None:
                images.append(image)
            else:
                images.append(self._placeholder_tile())
        return images

    def _load_first_tile_image(
        self, descriptor: MapDescriptor, names: Sequence[str]
    ) -> Optional[QtGui.QImage]:
        for name in names:
            image = self._load_tile_image(descriptor, name)
            if image is not None:
                return image
        return None

    def _load_tile_image(
        self, descriptor: MapDescriptor, relative: str
    ) -> Optional[QtGui.QImage]:
        relative_path = Path(relative)
        search_paths = [descriptor.world_path / relative_path]
        if self._data_root is not None:
            search_paths.append(self._data_root / relative_path)
    ...
    def _placeholder_tile(self) -> QtGui.QImage:
        image = QtGui.QImage(2, 2, QtGui.QImage.Format_RGBA8888)
        image.fill(QtGui.QColor(96, 96, 96))
        return image
...
class TerrainView(QOpenGLWidget):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        self._base_color = np.array([1.0, 1.0, 1.0], dtype=np.float32)
        ...
        self._tile_layer1_tex = 0
        self._tile_layer2_tex = 0
        self._tile_alpha_tex = 0
        self._tile_atlas_tex = 0
        self._tile_available = False
        self._atlas_cols = 1
        self._atlas_rows = 1
        self._atlas_step = (1.0, 1.0)
        self._tile_count = 0
    ...
    def paintGL(self) -> None:
        ...
        try:
            ...
            terrain_size = float(self._terrain.size if self._terrain is not None else TERRAIN_SIZE)
            GL.glUniform1f(GL.glGetUniformLocation(self._terrain_program, "u_terrain_size"), terrain_size)
            GL.glUniform1f(
                GL.glGetUniformLocation(self._terrain_program, "u_tile_count"),
                float(max(self._tile_count, 1)),
            )
            GL.glUniform2f(
                GL.glGetUniformLocation(self._terrain_program, "u_atlas_step"),
                self._atlas_step[0],
                self._atlas_step[1],
            )
            GL.glUniform2f(
                GL.glGetUniformLocation(self._terrain_program, "u_atlas_counts"),
                float(self._atlas_cols),
                float(self._atlas_rows),
            )
            has_tiles = 1 if self._tile_available else 0
            GL.glUniform1i(GL.glGetUniformLocation(self._terrain_program, "u_has_mapping"), has_tiles)
            if has_tiles:
                GL.glActiveTexture(GL.GL_TEXTURE0)
                GL.glBindTexture(GL.GL_TEXTURE_2D, self._tile_atlas_tex)
                GL.glUniform1i(GL.glGetUniformLocation(self._terrain_program, "u_tile_atlas"), 0)
                GL.glActiveTexture(GL.GL_TEXTURE1)
                GL.glBindTexture(GL.GL_TEXTURE_2D, self._tile_layer1_tex)
                GL.glUniform1i(GL.glGetUniformLocation(self._terrain_program, "u_tile_layer1"), 1)
                GL.glActiveTexture(GL.GL_TEXTURE2)
                GL.glBindTexture(GL.GL_TEXTURE_2D, self._tile_layer2_tex)
                GL.glUniform1i(GL.glGetUniformLocation(self._terrain_program, "u_tile_layer2"), 2)
                GL.glActiveTexture(GL.GL_TEXTURE3)
                GL.glBindTexture(GL.GL_TEXTURE_2D, self._tile_alpha_tex)
                GL.glUniform1i(GL.glGetUniformLocation(self._terrain_program, "u_tile_alpha"), 3)
            ...
            tex_loc = GL.glGetAttribLocation(self._terrain_program, "a_texcoord")
            tile_loc = GL.glGetAttribLocation(self._terrain_program, "a_tilecoord")
            ...
            GL.glBindBuffer(GL.GL_ARRAY_BUFFER, self._terrain_tbo)
            GL.glEnableVertexAttribArray(tex_loc)
            GL.glVertexAttribPointer(tex_loc, 2, GL.GL_FLOAT, GL.GL_FALSE, 0, None)
            GL.glBindBuffer(GL.GL_ARRAY_BUFFER, self._terrain_tilebo)
            GL.glEnableVertexAttribArray(tile_loc)
            GL.glVertexAttribPointer(tile_loc, 2, GL.GL_FLOAT, GL.GL_FALSE, 0, None)
            ...
            GL.glDisableVertexAttribArray(tile_loc)
    ...
    def _upload_terrain(self, data: TerrainData) -> None:
        ...
        tilecoords = data.tilecoords.astype(np.float32)
        ...
        self._terrain_tbo = GL.glGenBuffers(1)
        GL.glBindBuffer(GL.GL_ARRAY_BUFFER, self._terrain_tbo)
        GL.glBufferData(GL.GL_ARRAY_BUFFER, texcoords.nbytes, texcoords, GL.GL_STATIC_DRAW)
        self._terrain_tilebo = GL.glGenBuffers(1)
        GL.glBindBuffer(GL.GL_ARRAY_BUFFER, self._terrain_tilebo)
        GL.glBufferData(GL.GL_ARRAY_BUFFER, tilecoords.nbytes, tilecoords, GL.GL_STATIC_DRAW)
    ...
    def _upload_tiles(self, data: TerrainData) -> None:
        self._delete_tile_textures()
        self._tile_available = False
        self._tile_count = 0
        if not data.has_tile_mapping():
            return
        self._tile_available = True
        self._tile_count = data.tile_texture_count()
        layer1 = np.ascontiguousarray(data.tile_layer1)
        layer2 = np.ascontiguousarray(data.tile_layer2)
        alpha_bytes = np.rint(np.clip(data.tile_alpha, 0.0, 1.0) * 255.0).astype(np.uint8)
        prev_alignment = GL.glGetIntegerv(GL.GL_UNPACK_ALIGNMENT)
        if isinstance(prev_alignment, (tuple, list)):
            prev_alignment = prev_alignment[0]
        GL.glPixelStorei(GL.GL_UNPACK_ALIGNMENT, 1)
        self._tile_layer1_tex = self._create_index_texture(layer1)
        self._tile_layer2_tex = self._create_index_texture(layer2)
        self._tile_alpha_tex = self._create_alpha_texture(alpha_bytes)
        atlas_tex, cols, rows, step = self._create_tile_atlas(data.tile_images)
        self._tile_atlas_tex = atlas_tex
        self._atlas_cols = cols
        self._atlas_rows = rows
        self._atlas_step = step
        GL.glPixelStorei(GL.GL_UNPACK_ALIGNMENT, int(prev_alignment))

    def _create_index_texture(self, array: np.ndarray) -> int:
        ...
        data_ptr = np.ascontiguousarray(array)
        GL.glTexImage2D(
            GL.GL_TEXTURE_2D,
            0,
            GL.GL_LUMINANCE,
    ...
    def _create_alpha_texture(self, array: np.ndarray) -> int:
        texture = GL.glGenTextures(1)
        GL.glBindTexture(GL.GL_TEXTURE_2D, texture)
        GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER, GL.GL_NEAREST)
        GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MAG_FILTER, GL.GL_NEAREST)
    ...
    def _create_tile_atlas(
        self, images: Sequence[QtGui.QImage]
    ) -> Tuple[int, int, int, Tuple[float, float]]:
        if not images:
            placeholder = QtGui.QImage(64, 64, QtGui.QImage.Format_RGBA8888)
            placeholder.fill(QtGui.QColor(128, 128, 128))
            images = [placeholder]
        widths = [img.width() for img in images if not img.isNull()]
        heights = [img.height() for img in images if not img.isNull()]
        tile_w = max(widths) if widths else 64
        tile_h = max(heights) if heights else 64
        tile_w = max(tile_w, 32)
        tile_h = max(tile_h, 32)
        ...
        for idx in range(count):
            if idx < len(images):
                ...
                img = QtGui.QImage(tile_w, tile_h, QtGui.QImage.Format_RGBA8888)
                img.fill(QtGui.QColor(128, 128, 128))
            array = self._image_to_array(img, tile_w, tile_h)
            row = idx // cols
            col = idx % cols
            y0 = row * tile_h
            x0 = col * tile_w
            atlas[y0:y0 + tile_h, x0:x0 + tile_w, :] = array
        texture = GL.glGenTextures(1)
        GL.glBindTexture(GL.GL_TEXTURE_2D, texture)
        ...
        atlas_data = np.ascontiguousarray(atlas)
        GL.glTexImage2D(
            GL.GL_TEXTURE_2D,
            0,
            GL.GL_RGBA,
        ...
        GL.glBindTexture(GL.GL_TEXTURE_2D, 0)
        step = (1.0 / cols, 1.0 / rows)
        return texture, cols, rows, step

    def _image_to_array(self, image: QtGui.QImage, width: int, height: int) -> np.ndarray:
        if image.isNull():
            temp = QtGui.QImage(width, height, QtGui.QImage.Format_RGBA8888)
            temp.fill(QtGui.QColor(128, 128, 128))
            image = temp
        converted = image.convertToFormat(QtGui.QImage.Format_RGBA8888)
        if converted.width() != width or converted.height() != height:
            converted = converted.scaled(width, height, QtCore.Qt.IgnoreAspectRatio, QtCore.Qt.SmoothTransformation)
        ptr = converted.bits()
        ptr.setsize(converted.width() * converted.height() * 4)
        array = np.frombuffer(ptr, dtype=np.uint8).reshape((converted.height(), converted.width(), 4))
        return np.flipud(array.copy())

    def _delete_tile_textures(self) -> None:
        for texture in (self._tile_layer1_tex, self._tile_layer2_tex, self._tile_alpha_tex, self._tile_atlas_tex):
            if texture:
    ...
    def _release_buffers(self) -> None:
        ...
        if self._terrain_nbo:
            ...
            self._terrain_nbo = 0
        if self._terrain_tbo:
            GL.glDeleteBuffers(1, [self._terrain_tbo])
            self._terrain_tbo = 0
        if self._terrain_tilebo:
            GL.glDeleteBuffers(1, [self._terrain_tilebo])
            self._terrain_tilebo = 0
        ...
        terrain_fs = """
            ...
            void main() {
                ...
                if (u_has_mapping != 0) {
                    ...
                    float idx1 = texture2D(u_tile_layer1, lookup).r * 255.0;
                    float idx2 = texture2D(u_tile_layer2, lookup).r * 255.0;
                    float alpha = texture2D(u_tile_alpha, lookup).r;
                    ...
                    vec2 atlas_step = u_atlas_step;
                    float col1 = mod(tileIdx1, cols);
                    float row1 = floor(tileIdx1 / cols);
                    vec2 base1 = vec2(col1, row1) * atlas_step;
                    vec3 tex1 = texture2D(u_tile_atlas, base1 + frac_uv * atlas_step).rgb;
                    vec3 texColor = tex1;
                    if (alpha > 0.001 && tileIdx2 >= 0.0 && tileIdx2 < u_tile_count) {
                        float col2 = mod(tileIdx2, cols);
                        float row2 = floor(tileIdx2 / cols);
                        vec2 base2 = vec2(col2, row2) * atlas_step;
                        vec3 tex2 = texture2D(u_tile_atlas, base2 + frac_uv * atlas_step).rgb;
...
class MapWalkerWindow(QtWidgets.QMainWindow):
    ...
    def _update_status(self) -> None:
        ...
        tile_info = "-"
        tile = terrain.tile_indices_at(x, z)
        if tile is not None:
            layer1, layer2, alpha = tile
            if 0 <= layer2 < terrain.tile_texture_count() and alpha > 0.001:
                tile_info = f"{layer1}/{layer2} α={alpha:.2f}"
            else:
                tile_info = f"{layer1}"
        self.status_label.setText(
            f"Pos: ({x:8.1f}, {y:8.1f}, {z:8.1f})  |  Altura terreno: {terrain.sample_height(x, z):.1f}"
            f"  |  Attr: {attr_text}  |  Tile: {tile_info}"
        )
